...
public class SongProcessingService : ISongProcessingService
{
    ...
    public async Task<JObject> FormatSong(JToken data)
    {
        try
        {
            var encryptedMediaUrl = data["encrypted_media_url"]?.ToString();
            if (!string.IsNullOrEmpty(encryptedMediaUrl)) data["media_url"] = DecryptUrl(encryptedMediaUrl);

            var quality = data["320kbps"]?.ToString();
            var mediaUrl = data["media_url"]?.ToString();
            if (mediaUrl != null)
            {
                if (quality != "true") mediaUrl = mediaUrl.Replace(HighQualitySuffix, MediumQualitySuffix);
        ...
        {
            var mediaPreviewUrl = data["media_preview_url"]?.ToString();
    ...
    public async Task<JObject> FormatPlaylist(JToken data)
    {
        ...
        foreach (var song in data["songs"] ?? Enumerable.Empty<JToken>()) await FormatSong(song);
    ...
    {
        var restoredUrl = previewUrl.Replace("preview", "aac");
