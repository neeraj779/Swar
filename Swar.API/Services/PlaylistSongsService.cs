...
public class PlaylistSongsService : IPlaylistSongsService
{
    ...
    public PlaylistSongsService(IPlaylistSongsRepository playlistSongRepository, IPlaylistRepository playlistRepository,
        IRepository<int, User> userRepository, ILogger<PlaylistSongsService> logger)
    {
    ...
    public async Task<PlaylistSongsReturnDTO> AddSongToPlaylist(int userId, AddSongToPlaylistDTO addSongToPlaylistDTO)
    {
        ...
        var playlistSongExists =
            await _playlistSongRepository.GetByCompositeKey(addSongToPlaylistDTO.PlaylistId,
        ...
        if (playlistSongExists != null)
        {
            _logger.LogInformation(
                $"User {userId} tried to add song {addSongToPlaylistDTO.SongId} to playlist {addSongToPlaylistDTO.PlaylistId} but it already exists.");
        ...
        _logger.LogInformation(
            $"User {userId} added song {addSongToPlaylistDTO.SongId} to playlist {addSongToPlaylistDTO.PlaylistId}");
    ...
    public async Task<PlaylistSongsDTO> GetAllSongsInUserPlaylist(int userId, string publicId)
    {
        ...
        var playlistInfo = new PlaylistInfoDTO
        {
            UserId = user.UserId,
    ...
    public async Task<PlaylistSongsReturnDTO> RemoveSongFromPlaylist(int userId, int playlistId, string songId)
    {
        ...
        {
            _logger.LogInformation(
                $"User {userId} tried to remove song {songId} from playlist {playlistId} but it does not exist.");
    ...
    private async Task<(User user, Playlist playlist)> ValidateUserAndPlaylist(int userId, object playlistIdentifier,
        bool isGetRequest = false)
    {
        var playlist = playlistIdentifier switch
        {
        ...
        if (userId != 0)
        {
            ...
            if (user.UserStatus != UserStatusEnum.UserStatus.Active) throw new InactiveAccountException();
        }
