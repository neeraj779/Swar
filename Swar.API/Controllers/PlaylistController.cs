...
public class PlaylistController : ControllerBase
{
    ...
    public async Task<IActionResult> CreatePlaylist(AddPlaylistDTO addPlaylistDTO)
    {
        ...
        catch (InactiveAccountException ex)
        {
            return StatusCode(StatusCodes.Status403Forbidden,
                new ErrorModel { Status = StatusCodes.Status403Forbidden, Message = ex.Message });
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError,
                new ErrorModel { Status = StatusCodes.Status500InternalServerError, Message = ex.Message });
        }
    }
    ...
    public async Task<IActionResult> DeletePlaylist(int playlistId)
    {
        ...
        catch (EntityNotFoundException ex)
        {
            return StatusCode(StatusCodes.Status404NotFound,
                new ErrorModel { Status = StatusCodes.Status404NotFound, Message = ex.Message });
        }
    ...
    public async Task<IActionResult> GetAllPlaylists()
    {
        ...
        catch (EntityNotFoundException ex)
        {
            return StatusCode(StatusCodes.Status200OK,
                new ErrorModel { Status = StatusCodes.Status200OK, Message = ex.Message });
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError,
    ...
    [Authorize(Roles = "Admin, User")]
    public async Task<IActionResult> UpdatePlaylistPrivacy(int playlistId,
        UpdatePlaylistPrivacyDTO updatePlaylistPrivacyDTO)
    {
        try
        {
            var userId = Convert.ToInt32(User.FindFirstValue(ClaimTypes.NameIdentifier));
            var result =
                await _playlistService.UpdatePlaylistPrivacy(userId, playlistId, updatePlaylistPrivacyDTO.IsPrivate);
