...
public class PlayHistoryServiceTests
{
    ...
    public void SetUp()
    {
        _dbContextOptions = new DbContextOptionsBuilder<SwarContext>()
            .UseInMemoryDatabase("InMemoryPlayHistoryDb")
            .Options;
    ...
    public async Task GetSongHistory_ValidUser_ReturnsHistory()
    {
        ...
        var p1 = new PlayHistory { UserId = user.UserId, SongId = "song1", PlayedAt = DateTime.Now.AddMinutes(-10) };
        var p2 = new PlayHistory { UserId = user.UserId, SongId = "song2", PlayedAt = DateTime.Now.AddMinutes(-5) };
        await _playHistoryRepository.Add(p1);
        await _playHistoryRepository.Add(p2);
        await _dbContext.SaveChangesAsync();

        // Act
        var result = await _playHistoryService.GetSongHistory(user.UserId, false);
    ...
    {
        // Act & Assert
        Assert.ThrowsAsync<EntityNotFoundException>(() => _playHistoryService.GetSongHistory(999, false));
    }
    ...
    public async Task GetSongHistory_UserInactive_ThrowsInactiveAccountException()
    {
        ...
        // Act & Assert
        Assert.ThrowsAsync<InactiveAccountException>(() => _playHistoryService.GetSongHistory(user.UserId, false));
    }
    ...
    public async Task GetSongHistory_UniqueTrue_ReturnsUniqueHistory()
    {
        ...
        var p1 = new PlayHistory { UserId = user.UserId, SongId = "song1", PlayedAt = DateTime.Now.AddMinutes(-10) };
        var p2 = new PlayHistory { UserId = user.UserId, SongId = "song1", PlayedAt = DateTime.Now.AddMinutes(-5) };
        await _playHistoryRepository.Add(p1);
        await _playHistoryRepository.Add(p2);
        await _dbContext.SaveChangesAsync();

        // Act
        var result = await _playHistoryService.GetSongHistory(user.UserId, true);
    ...
    public async Task GetAllUserHistory_ReturnsAllUsersHistory()
    {
        ...
        var p1 = new PlayHistory { UserId = user1.UserId, SongId = "song1", PlayedAt = DateTime.Now.AddMinutes(-10) };
        var p2 = new PlayHistory { UserId = user2.UserId, SongId = "song2", PlayedAt = DateTime.Now.AddMinutes(-5) };
